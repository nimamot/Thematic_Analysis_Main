...
def _code_json(outputs: dict) -> str:
    return json.dumps({"responses": [{"id": str(k), "output": v} for k, v in outputs.items()]})
...
@patch("agents.core.batch_open_coding.llm_invoke_with_skill")
def test_parse_failure_falls_back_to_single_review(mock_invoke, mock_validate, mock_open, _bw, _bs):
    reviews = [(1, "text one")]
